...
这里没有哪张表天然叫“Memory”。`messages` 保存可追溯的过去，`task_states` 保存当前，`profiles` 保存稳定事实，`memories` 保存需要按语义或时间重新发现的经验。数据结构只是前面那套时间尺度和作用域在存储层的投影。

衡量这套结构时，真正值得看的不是“记住了多少”，而是写入是否克制、需要时能否找到、无关内容会不会误召回、新旧事实冲突时能否选择正确版本。删除也必须贯穿正文、向量索引和缓存。多用户系统再多一条不可妥协的标准：跨用户、跨项目泄漏必须为零。
